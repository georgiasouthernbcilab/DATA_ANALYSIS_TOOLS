...
contactQuality = ['CQ', 'Overall']
electrode_locations = ['EEG.Cz', 'EEG.Fz', 'EEG.Fp1', 'EEG.F7', 'EEG.F3', 'EEG.FC1', 'EEG.C3', 'EEG.FC5', 'EEG.FT9', 'EEG.T7', 'EEG.CP5', 'EEG.CP1', 'EEG.P3', 'EEG.P7', 'EEG.PO9', 'EEG.O1', 'EEG.Pz', 'EEG.Oz', 'EEG.O2', 'EEG.PO10', 'EEG.P8', 'EEG.P4', 'EEG.CP2', 'EEG.CP6', 'EEG.T8', 'EEG.FT10', 'EEG.FC6', 'EEG.C4', 'EEG.FC2', 'EEG.F4', 'EEG.F8', 'EEG.Fp2']
...
class Participant(Operations):
    ...
    def create_data_frame(self, file, filter=None):
        ...
        df = pd.read_csv(file_path, skiprows=1,index_col='Timestamp')
        if filter:
        ...
        for root, dirs, files in os.walk(self.directory):
            for file in files:
                if "Baseline" in file:
                    ...
                    self.baseline_raw_eeg_df = self.create_data_frame(file, electrode_locations)
                    ...
                    self.excited_raw_eeg_df = self.create_data_frame(file, electrode_locations)
                    
                elif 'From_videos_1_excited_2_sports_NBA' in file:
                    self.excited_pow_df = self.create_data_frame(file, column_categories)
                    self.excited_raw_eeg_dff = self.create_data_frame(file, electrode_locations)
                    ...
                    self.happy_raw_eeg_df = self.create_data_frame(file, electrode_locations)
                    ...
                    self.motivated_raw_eeg_df = self.create_data_frame(file, electrode_locations)
                    ...
                    self.relaxed_raw_eeg_df = self.create_data_frame(file, electrode_locations)
                    ...
                    self.sad_raw_eeg_df = self.create_data_frame(file, electrode_locations)
    ...
    def compute_all_aggregated_statistics(self):
        dataframes = {
            'baseline_raw_eeg_df': self.baseline_raw_eeg_df,
            'excited_raw_eeg_df': self.excited_raw_eeg_df,
            'happy_raw_eeg_df': self.happy_raw_eeg_df,
            'motivated_raw_eeg_df': self.motivated_raw_eeg_df,
            'relaxed_raw_eeg_df': self.relaxed_raw_eeg_df,
            'sad_raw_eeg_df': self.sad_raw_eeg_df,
            'fearful_raw_eeg_df': self.fearful_raw_eeg_df,
            'disgust_raw_eeg_df': self.disgust_raw_eeg_df,
            'angry_raw_eeg_df': self.angry_raw_eeg_df,
